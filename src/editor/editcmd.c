...
edit_suggest_current_word (WEdit * edit)
{
    ...
    if (!aspell_check (match_word, (int) word_len))
    {
        ...
        if (res != 0)
        {
            ...
            retval = editcmd_dialog_spell_suggest_show (edit, match_word, &new_word, suggest);
            cp_word = new_word;
...
void
edit_set_spell_lang (WEdit * edit)
{
    GArray *lang_list;
    ...
    if (aspell_get_lang_list (lang_list) != 0)
    {
        ...
        lang = editcmd_dialog_lang_list_show (edit, lang_list);
        if (lang != NULL)
